...
<template>
    ...
    <div id="main" class="sponsorColumns">
        <div v-if="partners.length!=0">
            <GlowText v-if="partners.length == 1" text="PARTNER" font-size="var(--font-huge)" :color=$props.color shadow/>
    ...
    <div class="sponsors" v-if="bronze.length != 0">
        <HomeSponsorLogo v-for="sponsor in bronze" :key="sponsor.name" :src="sponsor.src" :url="sponsor.url" :name="sponsor.name" :height="sponsor.height" />
    </div>
</template>
